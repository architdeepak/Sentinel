"""
Live Metrics Dashboard for V7 Conversation Mode.

Displays a real-time window during the LLM conversation showing:
  - Detection metrics (from DetectionThread): drowsy score, perclos, blinks, etc.
...
Runs in its own thread so it doesn't block STT/LLM.
"""
...
class MetricsDashboard:
    """Live metrics display during LLM conversation."""

    def __init__(self, detection_thread, baselines=None):
        self._det = detection_thread
        self._baselines = baselines or {}
        self._running = False
        self._thread = None

        # Voice metrics (updated per turn from main thread)
    ...
    def update_voice(self, features):
        """Called from main thread after each voice extraction."""
    ...
    def start(self):
        """Start the dashboard window thread."""
        if self._running:
            return
        self._running = True
    ...
    def stop(self):
        """Stop the dashboard and close the window."""
        self._running = False
        if self._thread:
    ...
    def _render_loop(self):
        ...
        while self._running:
            ...
            # ── Footer ──
            cv2.putText(img, "ESC in main window to exit | Dashboard auto-closes after conversation",
                        (10, _H - 12), _FONT, 0.3, _GRAY, 1)

            cv2.imshow("Sentinel Dashboard", img)
            key = cv2.waitKey(200) & 0xFF  # ~5 FPS
            if key == 27:
                break
