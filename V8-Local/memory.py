...
class MemoryManager:
    ...
    def __init__(self, db_path=None):
        ...
        self._llm = None          # Set via set_llm() after LLMAssistant is ready
    ...
    def learn_driver_patterns(self, session_id):
        ...
        if self._llm is None:
            print("ℹ️ LLM not available — skipping pattern learning")
            return
...
Return [] if not enough data for meaningful patterns.""")
    ...
    def set_llm(self, llm):
        """Inject the shared Llama instance from LLMAssistant.

        Called by main.py after LLMAssistant is initialized so both share
        the same model — avoids loading a second GGUF on the RPi.
        """
        self._llm = llm
...
[
  ...
  {{"type": "driving_routine", "value": "commutes 45 mins each way"}}
        ...
        if self._llm is None:
            print("ℹ️ LLM not available — skipping fact extraction")
            return 0
