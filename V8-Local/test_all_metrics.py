#!/usr/bin/env python3
"""
Sentinel V8-Local — Full Metrics Test Suite
============================================
Single interactive script covering every science fair metric category.
Stack: LOCAL — Granite-1B (llama-cpp) / Vosk STT / espeak-ng TTS

Sections:
  0  Setup & Calibration          (~2 min)
  1  Detection Performance        (~5 min)  3 behavior cycles × alert+drowsy
  2  Conversation — New Driver    (~4 min)  2 turns, latency + voice + memory
  3  Conversation — Returning     (~2 min)  1 turn, recall accuracy
  4  Conversation Behavior        (~3 min)  Termination accuracy + false exits
...
# Known facts to share across 2 turns — used to score extraction accuracy
# Turn 1: name + job combined; Turn 2: destination
KNOWN_FACTS = {1: "name and job", 2: "destination today"}
...
def _new_state() -> dict:
    _W = 200   # local — does not shadow global W
    return {
        'ear_window':        deque(maxlen=_W),
        'pitch_window':      deque(maxlen=_W),
        'closed_window':     deque(maxlen=_W),
...
def section_0_setup() -> dict:
    ...
    # ── MetricReasoner (local pure-Python thresholds — instant) ──────────────
    try:
        reasoner = MetricReasoner()
        ok("MetricReasoner (local thresholds)")
        ctx['reasoner'] = reasoner
    except Exception as e:
        warn(f"MetricReasoner failed: {e}")
        ctx['reasoner'] = None

    # ── LLMAssistant (Granite 1B GGUF — model load takes ~20-30 s) ───────────
    step("Loading Granite 1B model — this takes ~20-30 seconds...")
    try:
    ...
    if ctx.get('cap') and ctx.get('face_mesh'):
        ...
        while time.perf_counter() - t_cal < 5.0:
            ...
            if not ret:
                continue
            # Camera is 320x240 = PROC_W×H; resize is typically a no-op
        ...
        for phase_name, true_label, duration, label_int in [
            ...
            while True:
                ...
                if frame_count % FRAME_SKIP == 0:
                    ...
                    # Microsleep — instant trigger (no API latency in local version)
                    ...
                    # Reasoner gate — instant (pure-Python thresholds, 3 s interval)
...
def _run_conversation(ctx: dict, n_turns: int, turn_reminders: list,
    ...
    logger = MetricsLogger(trial_id=session_label,
                           driver_type="new" if session_count == 0 else "returning")
    ...
    # V8-Local signature: start_conversation(det_ctx, baselines_str,
    #   session_count=, reasoner_context=, driver_history=, drowsy_freq=)
    ...
    for turn in range(1, n_turns + 1):
        ...
        # Voice features (audio_data is raw PCM bytes from Vosk)
        voice_ctx = None
        ...
        # Live latency summary (LLM-Full is the key metric for local model)
        t = logger.turns[-1]
        print(f"     STT:{t.get('stt_latency_ms','—')}ms | "
              f"LLM-1st:{t.get('groq_latency_ms','—')}ms | "
              f"LLM-Full:{t.get('groq_full_latency_ms','—')}ms | "
    ...
    # Write latency rows + export LLM responses for blind second-rater scoring
...
def section_2_new_driver(ctx: dict):
    ...
    _header("SYSTEM_LATENCY",
            ["Session", "Turn", "STT_ms", "LLM_First_ms",
             "LLM_Full_ms", "TTS_Gen_ms", "User_Speech_ms", "Total_ms"])
    ...
    step("New driver session — 2 turns  (local LLM: expect 10-30s per response)")
    info("Speak naturally when Sentinel asks questions.")
    info("Turn 1: mention your NAME and JOB / occupation")
    info("Turn 2: mention your DESTINATION today")
    pause("\n  Press ENTER to start the conversation...")

    _seed_baselines(ctx['memory'])

    reminders = [
        "  → Mention your NAME and JOB",
        "  → Mention your DESTINATION today",
    ]

    logger, turns_responded, session_id = _run_conversation(
        ctx, n_turns=2, turn_reminders=reminders,
    ...
    print(f"\n  Which of the 2 known fact groups did you mention?")
    print(f"  1 = name + job    2 = destination")
    shared_input = input("  → (e.g. '1 2' or '1'): ").strip()
    facts_shared = []
    ...
    # Extraction accuracy — check each word in the known fact label against db types
    db_types = {ft.lower() for ft, _, _, _ in facts_in_db}
    recalled = sum(
        1 for f in facts_shared
        if any(part in dt for part in f.split() for dt in db_types)
    )
    ...
    ctx['s1_total_turns']     = 2

    ok(f"Extraction accuracy: {extr_acc}  ({recalled}/{len(facts_shared)} fact groups found in DB)")
    logger.save()
...
def section_3_returning_driver(ctx: dict):
    ...
    step("Returning driver session — 1 turn  (same DB as Session 1)")
    ...
    # Recall accuracy: how many Session 1 fact groups are still in the DB?
    ...
    ok(f"Recall accuracy: {recall_acc}  ({recalled}/{len(facts_s1)} Session 1 fact groups found)")
    logger.save()
...
def section_4_behavior(ctx: dict):
    ...
    info("(Local LLM — expect longer latency before [RECOVERED] appears)")
            ...
            if "[RECOVERED]" in resp:
                ...
                if u3:
                    resp2 = llm.get_response_streaming(
                        user_message=u3,
                        detection_context=format_detection_for_llm(FAKE_ALERT_METRICS))
                    tts.wait_until_done()
    ...
    false_exits = 0
    for phrase, _ in CONFOUNDING_PHRASES:
        phrase_lc = phrase.lower()
        hit = any(kw in phrase_lc for kw in EXIT_KEYWORDS)
        if hit:
            matched = next(kw for kw in EXIT_KEYWORDS if kw in phrase_lc)
...
def section_5_pvtb(ctx: dict):
    ...
    _header("PVT_B", ["Stimulus", "Interval_s", "RT_ms", "Is_Lapse"])
...
def section_6_output(ctx: dict):
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(f"# Sentinel V8-Local — Full Metrics Test\n")
        f.write(f"# Stack: LOCAL (Granite-1B / Vosk / espeak-ng)\n")
    ...
    print("═" * W)
    print("  FINAL RESULTS SUMMARY  (V8-Local — Granite-1B / Vosk / espeak-ng)")
    print("═" * W)
...
def main():
    print("\n" + "═" * W)
    print("  SENTINEL V8-LOCAL — FULL METRICS TEST SUITE")
    print("  Stack: Granite-1B / Vosk / espeak-ng  |  ~20 minutes")
    print("═" * W)
    print(f"\n  Output → {OUTPUT_FILE}")
